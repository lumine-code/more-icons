...
const LUMINE_ICON = fs
  .readFileSync(path.join(__dirname, "..", "assets", "lumine.svg"))
  .toString("base64");
...
exports.ruleFor = function ruleFor(className, contract = false) {
  ...
  // The upstream Devicons font contains Atom's mark at the slot its table
  // assigned to Atom-specific paths. Renaming that slot during the Lumine
  // rebrand changed only its label, not its ink. Render the real Lumine mark
  // as a mask so it inherits both the selected palette colour and the theme's
  // text colour when coloured icons are disabled.
  if (className === `${GLYPH_PREFIX}lumine-icon`) {
    const image = `url("data:image/svg+xml;base64,${LUMINE_ICON}")`;
    return (
      `content: ""; background-color: currentColor; ` +
      `-webkit-mask: ${image} center / contain no-repeat; ` +

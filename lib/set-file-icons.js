...
exports.ruleFor = function ruleFor(className, contract = false) {
  ...
  // Contract emission. glyphs.json records size/top only where a glyph
  // deviates from its font's upstream mixin default (fonts.json), so resolve
  // to the absolute upstream value first. The size is emitted as a ratio of
  // the contract box so a resized icon column scales the glyphs with it.
  // Upstream's per-font base `top` corrected the old baseline frame, which the
  // box's content-area centring absorbs — only the glyph's own deviation from
  // its font default carries over, plus the font's calibrated `nudge`.
  const font = FONTS[glyph.font] || { size: 16, top: 0, nudge: 0 };
  const size = glyph.size ?? font.size;
  const dy = font.nudge + (glyph.top ?? font.top) - font.top;

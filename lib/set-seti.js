...
function languageIdFor(filePath, languageIds) {
  const grammar = lumine.grammars.selectGrammar(filePath);
...
exports.resolve = function resolve(filePath, { directory = false, mode = "dark" } = {}) {
  ...
  const classes = id && theme.definitions[id] ? ["mi-icon", PREFIX + cssEscape(id)] : null;
  theme.cache.set(cacheKey, classes);
  return classes;
};

// The manifest format carries no positional data at all, so this single
// constant is Seti's whole positional story under the icon contract:
// calibrated once against the octicon reference position (whose ink rides
// 1px below box centre; Seti's is dead-centred), then frozen. No horizontal
// component — with the advance centred in the contract box, Seti's ink-to-text
// gap already exceeds the octicon reference.
const NUDGE = { x: 0, y: 1 };
...
exports.ruleFor = function ruleFor(className, contract = false) {
  ...
  if (contract && (NUDGE.x || NUDGE.y)) declarations += ` translate: ${NUDGE.x}px ${NUDGE.y}px;`;
  return declarations;
};

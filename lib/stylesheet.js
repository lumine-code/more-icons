...
class Stylesheet {
  constructor() {
    ...
    this.rules = [];
    this.sourcePath = path.join(__dirname, "generated-icons.css");
    this.validationSheet = null;
  ...
  attach() {
    ...
    this.contract =
      ...
      "box";
    this.validationSheet = new document.defaultView.CSSStyleSheet();
    this.disposable = lumine.styles.addStyleSheet("", {
  ...
  publish() {
    if (!this.disposable) return;
    lumine.styles.addStyleSheet(this.rules.join("\n"), {
      sourcePath: this.sourcePath,
      context: "more-icons",
    });
  }

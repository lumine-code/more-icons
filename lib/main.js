...
function activateSet() {
  const name = atom.config.get("file-icons.set");
  const factory = SETS[name] || SETS["file-icons"];
  const next = factory();

  if (next.load) {
    const custom = atom.config.get("file-icons.customThemePath");
    try {
...
function iconClassForPath(filePath) {
  ...
  if (!coloured) {
    return classes.filter((className) => !className.startsWith("fi-c-"));
  }
  return classes;
}
...
module.exports = {
  activate() {
    ...
    activateSet();
    mode = currentMode();
    coloured = atom.config.get("file-icons.coloured");
    stylesheet.reset(set, mode);

    disposables.add(
      atom.config.onDidChange("file-icons.set", () => {
        activateSet();
        rebuild();
      }),
      atom.config.onDidChange("file-icons.customThemePath", () => {
        activateSet();
        rebuild();
      }),
      atom.config.onDidChange("file-icons.coloured", ({ newValue }) => {
        coloured = newValue;
        emitDidChange();
      }),
  ...
  // Only `file-icons.classes` is provided. Registering `file-icons.elements`

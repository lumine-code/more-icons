...
function styleElement() {
  return document.head.querySelector("style[data-file-icon-themes]");
}
...
describe("activation", () => {
  ...
  beforeEach(() => {
    ...
    waitsForPromise(() => atom.packages.activatePackage("file-icon-themes"));

    runs(() => {
      service = atom.packages.getActivePackage("file-icon-themes").mainModule.provideFileIcons();
    });
  });

  afterEach(() => {
    waitsForPromise(() => atom.packages.deactivatePackage("file-icon-themes"));
  });
  ...
  it("installs a single stylesheet holding the fonts and shared geometry", () => {
    ...
    expect(ruleTexts().some((rule) => rule.includes(".icon.fit-icon::before"))).toBe(true);
  });

  it("exposes the colour palette as custom properties", () => {
    const root = ruleTexts().find((rule) => rule.startsWith(":root"));
    expect(root).toContain("--fit-medium-yellow:");
    expect(root).toContain("--fit-dark-blue:");
  });

  it("writes a glyph rule the first time a path needs one", () => {
    const before = ruleTexts().length;
    expect(ruleTexts().some((rule) => rule.includes("fit-g-python-icon"))).toBe(false);

    expect(service.iconClassForPath("/p/script.py")).toContain("fit-g-python-icon");

    expect(ruleTexts().length).toBeGreaterThan(before);
    expect(ruleTexts().some((rule) => rule.includes("fit-g-python-icon"))).toBe(true);
  });
  ...
  it("drops the colour class when colouring is turned off", () => {
    expect(service.iconClassForPath("/p/main.js")).toContain("fit-c-medium-yellow");

    atom.config.set("file-icon-themes.coloured", false);
    const classes = service.iconClassForPath("/p/main.js");
    expect(classes).toContain("fit-g-js-icon");
    expect(classes.some((name) => name.startsWith("fit-c-"))).toBe(false);
  });

  it("rebuilds the stylesheet when the set changes", () => {
    expect(service.iconClassForPath("/p/main.js")).toContain("fit-g-js-icon");

    atom.config.set("file-icon-themes.set", "seti");

    expect(ruleTexts().some((rule) => rule.includes("fit-g-js-icon"))).toBe(false);
    ...
    expect(service.iconClassForPath("/p/main.js")).toContain("fit-s-_javascript");
  });
  ...
  it("notifies consumers when its answers change", () => {
    ...
    atom.config.set("file-icon-themes.coloured", true);
    expect(callback.calls.count()).toBe(2);
  });
  ...
  it("warns and keeps working when a custom theme folder is unusable", () => {
    ...
    atom.config.set("file-icon-themes.set", "seti");
    atom.config.set("file-icon-themes.customThemePath", path.join(__dirname, "fixtures"));
